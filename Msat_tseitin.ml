...
module MakeCNF = struct
  ...
  let make_atom p = Lit (F.transparent p)
  ...
  let rec sform f k = match f with
    ...
    | Comb (Not, [Lit a]) -> k (make_atom (F.neg (F.opaque a)))
  ...
  *)
  ...
  let mk_proxy () = F.transparent (F.fresh ())
  ...
  let rec cnf f = match f with
    | Lit a -> None, [a]
    | Comb (Not, [Lit a]) -> None, [F.transparent (F.neg (F.opaque a))]
  ...
  let cnf f =
    ...
    (* encore clauses that make proxies in !acc_and equivalent to
       their clause *)
    let opaque_acc = fun (a,l) -> F.opaque a, List.map F.opaque l in
    let acc =
      List.fold_left
        (fun acc (p,l) ->
           proxies := p :: !proxies;
           ...
           let cl, acc =
             List.fold_left
               ...
               ([p],acc) l in
           cl :: acc
        ) (List.map (List.map F.opaque) acc) (List.map opaque_acc !acc_and)
    in
      ...
      List.fold_left
        (fun acc (p,l) ->
           ...
           let acc = List.fold_left (fun acc a -> [p; F.neg a]::acc)
               acc l in
           (F.neg p :: l) :: acc
        ) acc (List.map opaque_acc !acc_or)
    in
    acc
